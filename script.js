...
function loadFromUrl() {
    ...
    try {
        ...
        // Carica direttamente i dati dal link senza chiedere conferma al cliente
...
// === GALLERIA LISTINO ===
// Prova diversi percorsi (case-sensitive su GitHub Pages) e mostra un messaggio
// chiaro se non trova nulla. Per aggiungere immagini, basta inserirle nella
// stessa cartella di index.html oppure in una sottocartella "listino/".
function loadGallery() {
    ...
    // Costruisci la lista dei nomi attesi
    const fileNames = ['Copertina.jpg'];
    for (let i = 1; i <= 17; i++) {
        if (i !== 4) fileNames.push(`Pagina (${i}).jpg`);
    }

    // Possibili percorsi base dove cercare le immagini
    const basePaths = ['', 'listino/', 'images/', 'img/', 'foto/'];

    let totalLoaded = 0;
    let totalAttempts = fileNames.length;
    let completedAttempts = 0;

    // Funzione che prova in ordine tutti i basePaths per un singolo file
    function tryLoadImage(fileName) {
        return new Promise((resolve) => {
            let pathIdx = 0;
            const tryNext = () => {
                if (pathIdx >= basePaths.length) {
                    resolve(null);
                    return;
                }
                const fullPath = basePaths[pathIdx] + fileName;
                const testImg = new Image();
                testImg.onload = () => resolve(fullPath);
                testImg.onerror = () => { pathIdx++; tryNext(); };
                testImg.src = encodeURI(fullPath);
            };
            tryNext();
        });
    }

    // Crea un placeholder per mantenere l'ordine; poi sostituisce con l'immagine trovata
    const placeholders = fileNames.map((name, idx) => {
    ...
    fileNames.forEach((fileName, idx) => {
        tryLoadImage(fileName).then(workingPath => {
            completedAttempts++;
            if (workingPath) {
                const img = document.createElement('img');
                img.src = encodeURI(workingPath);
                img.alt = fileName;
                img.loading = 'lazy';
                placeholders[idx].replaceWith(img);
                totalLoaded++;
            }
            // Quando tutti i tentativi sono finiti, controlla se la galleria è vuota
            if (completedAttempts === totalAttempts && totalLoaded === 0) {
                showGalleryError();
            }
        });
    });
...
function showGalleryError() {
    const container = document.getElementById('gallery-container');
    container.innerHTML = `
        <div style="color:#fff; background:#222; padding:30px; border-radius:8px; text-align:center; max-width:600px; line-height:1.6;">
            <h2 style="color:#e74c3c; margin-top:0;">⚠ Listino non trovato</h2>
            <p>Le immagini del listino non sono state caricate.</p>
            <p style="font-size:14px;">Verifica che i file JPG siano nella stessa cartella di <code>index.html</code> con questi nomi esatti (rispettando maiuscole/minuscole):</p>
            <div style="text-align:left; background:#111; padding:12px; border-radius:4px; font-family:monospace; font-size:12px; margin:12px 0; color:#9cdcfe;">
                Copertina.jpg<br>
                Pagina (1).jpg<br>
                Pagina (2).jpg<br>
                Pagina (3).jpg<br>
                Pagina (5).jpg ... fino a Pagina (17).jpg
            </div>
            <p style="font-size:13px;">Oppure mettile in una sottocartella <code>listino/</code>, <code>images/</code>, <code>img/</code> o <code>foto/</code>.</p>
            <button onclick="retryGallery()" style="margin-top:10px; padding:10px 20px; background:#27ae60; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:bold;">🔄 Riprova</button>
        </div>
    `;
}
