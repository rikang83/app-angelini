...
function buildStateObject() {
    const data = { v: 1, fields: [], signatures: {} };
...
function loadFromUrl() {
    ...
    try {
        ...
        // Vai automaticamente al contratto
        setTimeout(() => {
            showDoc('contratto');
            alert('✓ Contratto caricato dal link. Compila il riquadro firme e poi premi "CONDIVIDI" per rimandarlo.');
        }, 200);
        return true;
    } catch (e) {
...
function generatePDF() {
    ...
    const activeScreen = document.querySelector('.screen.active');
    snapshotInputValuesIntoDom(document);
    // Attendi che i font web (Tinos) siano caricati prima di renderizzare,
    // altrimenti html2canvas usa fallback (Arial) → grafica diversa dall'app.
    ...
    ready.then(() => {
        ...
        html2pdf().from(activeScreen).set(opt).toPdf().get('pdf').then((pdf) => {
            // Rimuove pagine vuote che html2pdf aggiunge di troppo
